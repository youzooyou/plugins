...
skill is round-level and all-groups-together (see `codex-stream-review/skills/ccs/SKILL.md`'s own
"Convergence = 100% CLEAN" section): every group
...
## Round-1 focus text — the parallel-mode dimension sub-bullet
  ...
  - **Parallel mode (more than one group this round):** every group's Round-1 `--focus` shares the
    ...
    IDENTICAL full diff (see `codex-stream-review/skills/ccs/SKILL.md`'s own "Determine review
    mode" section).
...
## Round-2+ focus text — per-group history construction
  ...
  - **Per-group History construction (parallel mode subtlety).** A group's round-2+ `--focus`
    ...
    *different* group's finding — a fix for one review dimension can regress another, this is the
    whole-flow principle (`codex-stream-review/skills/ccs/SKILL.md`'s own Phase 2) applied ACROSS
    dimensions, not only within one; and (b)
    ...
    tagged by this group's own `group` value in `groups[]` (see `SKILL.md`'s own "Review history
    log" section), not
    ...
    still resumed and re-checked, never dropped from the round (see `SKILL.md`'s own
    "Convergence = 100% CLEAN" section).

## JSONL field: `groups`

- `groups`: **added only for a parallel round — more than one group dispatched this round (see
  `codex-stream-review/skills/ccs/SKILL.md`'s own "Determine review mode" section).** Each
  dispatched group runs its own separate wrapper call with
  ...
  field entirely**, exactly as shown in `SKILL.md`'s own "Review history log" example. **Only when this round actually
  ...
  `target`/etc., exactly as in `SKILL.md`'s own full single-reviewer example.)
  ...
  `codex_review` is an AGGREGATED verdict/findings, worst-case-wins exactly like `SKILL.md`'s own
  `coverage_source` merge: `verdict` is `"ISSUES"` if ANY group's own verdict was `"ISSUES"`
